...
def login_view(request):
    global i, addr, passwrd, redirect_page
    login = False
    if request.method == 'POST':
        ...
        try:
            ...
            texttospeech(
                "Congratulations. You have logged in successfully. You will now be redirected to the menu page.", file + i)
            i = i + str(1)
            login = True
            return JsonResponse({'result': 'success'})
            # redirect_page = 'homepage/options.html'
            return redirect(request, 'homepage/options.html')
        except:
            texttospeech("Invalid Login Details. Please try again.", file + i)
            i = i + str(1)
            return JsonResponse({'result': 'failure'})
            # redirect_page = 'homepage/login.html'
